...
This example provides full analysis of Darcy flow datasets generated by
...
def _compute_field_statistics(fields: jax.Array) -> dict[str, float]:
    """Compute full statistics for field data."""
    return {
...
def _analyze_spatial_patterns(inputs: jax.Array, outputs: jax.Array) -> dict[str, Any]:
    ...
    output_grad_magnitude = jnp.sqrt(jnp.square(output_grad_x) + jnp.square(output_grad_y))
    ...
    return {
        ...
        "gradient_correlation": float(
            jnp.corrcoef(input_grad_magnitude.flatten(), output_grad_magnitude.flatten())[0, 1]
        )
...
# %%
def _create_resolution_comparison_plots(results: dict[str, Any], save_path: str | None) -> None:
    ...
    input_ranges = [datasets[res]["input_stats"]["dynamic_range"] for res in resolutions]
    output_ranges = [datasets[res]["output_stats"]["dynamic_range"] for res in resolutions]
    ...
    line1 = axes[1, 1].plot(resolutions, generation_times, "ro-", label="Generation Time (s)")
    ...
    plt.tight_layout()
    if save_path:
        plt.savefig(f"{save_path}_resolution_comparison.png", dpi=300, bbox_inches="tight")
    plt.show()


def _create_statistical_summary_plots(results: dict[str, Any], save_path: str | None) -> None:
    ...
    plt.tight_layout()
    if save_path:
        plt.savefig(f"{save_path}_statistical_summary.png", dpi=300, bbox_inches="tight")
    plt.show()


def _create_performance_analysis_plots(results: dict[str, Any], save_path: str | None) -> None:
    ...
    plt.tight_layout()
    if save_path:
        plt.savefig(f"{save_path}_performance_analysis.png", dpi=300, bbox_inches="tight")
    plt.show()
...
def create_visualization(
    ...
    """Create full visualization of analysis results."""
...
- See [FNO Darcy Full](../models/fno_darcy_comprehensive.md) for training with this data
"""
