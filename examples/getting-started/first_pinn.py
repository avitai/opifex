...
# %%
def exact_solution(x: jax.Array) -> jax.Array:
    """Analytical solution: u(x) = sin(pi*x)."""
    return jnp.sin(jnp.pi * x)


def source_term(x: jax.Array) -> jax.Array:
...
def main() -> dict[str, float | int]:
    """Solve the 1D Poisson equation with a PINN and return scalar metrics."""
    print("=" * 60)
    ...
    # Step 3: Create the PDE residual and solve.
    print()
    ...
    return {
        ...
        "training_time_s": float(result.training_time),
    }
