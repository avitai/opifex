#!/usr/bin/env python3
"""Comprehensive Neural Operator Comparative Benchmarking Study.

This script provides a complete comparative analysis of UNO, FNO, and SFNO
neural operators using the Opifex framework's benchmarking infrastructure.

Features:
- Multi-operator comparison (UNO vs FNO vs SFNO)
- Multiple dataset evaluation (Darcy, Burgers, Advection)
...
try:
    from opifex.data.datasets.burgers import BurgersEquationDataset
    from opifex.data.datasets.darcy_flow import DarcyFlowDataset
...
class NeuralOperatorComparativeStudy:
    ...
    def __init__(
        self,
        output_dir: str = "results/neural_operator_comparison",
    ...
    def generate_test_datasets(
        ...
        try:
            ...
            darcy_dataset = DarcyFlowDataset(
                n_train=800,
                n_test=200,
                resolution=resolution,
            )
            train_data = darcy_dataset.get_train_data()
            test_data = darcy_dataset.get_test_data()
            datasets["Darcy"] = {
        ...
        try:
            ...
            burgers_dataset = BurgersEquationDataset(
                n_train=800,
                n_test=200,
                resolution=resolution,
            )
            train_data = burgers_dataset.get_train_data()
            test_data = burgers_dataset.get_test_data()
            datasets["Burgers"] = {
                "x_train": train_data[0],
                "y_train": train_data[1],
                "x_test": test_data[0],
    ...
    def benchmark_operator(
        ...
        """
        logger.info(
            f"🏃 Benchmarking {operator_name} on {dataset_name} "
    ...
    def run_resolution_study(self):
        ...
        for resolution in self.resolution_sizes:
            ...
            if not operators:
                logger.warning(f"⚠️ No operators created for resolution {resolution}")
                continue

            # Generate datasets for this resolution
            datasets = self.generate_test_datasets(resolution)
            if not datasets:
                logger.warning(f"⚠️ No datasets generated for resolution {resolution}")
                continue
    ...
    def save_intermediate_results(self, resolution: int):
        ...
        try:
            ...
            if resolution_results:
                ...
                logger.warning(f"⚠️ No results to save for resolution {resolution}")
    ...
    def create_performance_plots(
        ...
        try:
            ...
            if execution_times:
                ...
                ax.boxplot(times_data, labels=operators)
    ...
    def perform_statistical_analysis(
        ...
        try:
            ...
            if len(operators) < 2:
                logger.warning("⚠️ Need at least 2 operators for comparison")
                return
...
def main():
    ...
    parser.add_argument(
        "--output-dir",
        default="benchmark_results/neural_operator_comparison",
