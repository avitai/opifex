...
# Initial (incorrect) guess for the coefficient (DeepXDE uses 2.0)
C_INIT = 2.0
...
# ## Run the example
#
# Builds collocation points and observations, trains the PINN while
# discovering the diffusion coefficient, evaluates against the exact
# solution, and saves the visualizations.


# %%
def main() -> dict[str, float | int]:
    """Train an inverse-diffusion PINN, recover the coefficient, and plot."""
    print("=" * 70)
    ...
    final_loss = losses[-1]
    discovered_c = float(pinn.coef)
    coef_rel_error = abs(discovered_c - C_TRUE) / C_TRUE
    print(f"Final loss: {final_loss:.6e}")
    print(f"Discovered C: {discovered_c:.6f}")
    print(f"True C:       {C_TRUE:.6f}")
    print(f"Relative error: {coef_rel_error * 100:.2f}%")
    ...
    print(f"  Discovered C:      {discovered_c:.6f}")
    print(f"  True C:            {C_TRUE:.6f}")
    print(f"  Parameter error:   {coef_rel_error * 100:.2f}%")
    print(f"  Relative L2 error: {l2_error:.6e}")
    ...
    return {
        ...
        "discovered_coefficient": discovered_c,
        "coefficient_relative_error": coef_rel_error,
