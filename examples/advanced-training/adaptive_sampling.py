...
class BurgersPINN(nnx.Module):
    ...
    def __init__(self, hidden_dims: list[int] | None = None, *, rngs: nnx.Rngs) -> None:
        super().__init__()
...
# %%
def compute_burgers_residual(pinn: BurgersPINN, xt: jax.Array, nu: float) -> jax.Array:
    """Compute Burgers equation residual: u_t + u*u_x - nu*u_xx = 0."""

    def u_scalar(xt_single: jax.Array) -> jax.Array:
    ...
    def residual_single(xt_single: jax.Array) -> jax.Array:
        ...
        def du_dx_fn(xt: jax.Array) -> jax.Array:
...
def main() -> dict[str, float | int]:
    ...
    def make_loss_fn(xt_domain: jax.Array):
        """Create loss function factory to avoid loop variable capture."""

        def loss_fn(model: BurgersPINN) -> jax.Array:
    ...
    for step in range(TRAINING_STEPS):
        ...
        if step > 0 and step % REFINE_FREQUENCY == 0:
            ...
            print(
                ...
                f"points={len(xt_current)}, max_res={max_res:.4e}"
            )
    ...
    uniform_history: dict[str, list[float]] = {
        "step": [],
        "loss": [],
        "max_residual": [],
    }
    ...
    @nnx.jit
    def train_step_uniform(pinn: BurgersPINN, opt: nnx.Optimizer) -> jax.Array:
    ...
    return {
        ...
        "adaptive_final_loss": float(adaptive_history["loss"][-1]),
        "uniform_final_loss": float(uniform_history["loss"][-1]),
