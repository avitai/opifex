...
All run logic lives in ``main()``: data generation, the closure-based loss
functions, GradNorm setup and training, the fixed-weights baseline,
evaluation, and both figure blocks.
"""


# %%
def main() -> dict[str, float | int]:
    """Train a heat-equation PINN with GradNorm and a fixed-weight baseline."""
    # Configuration
    SEED = 42
    N_COLLOCATION = 500
    N_BOUNDARY = 100
    ...
    # ------------------------------------------------------------------
    # Step 3: Define loss functions (close over the data above)
    # ------------------------------------------------------------------
    ...
    return {
        ...
        "l2_error_gradnorm": float(l2_gradnorm),
        "l2_error_fixed": float(l2_fixed),
        "final_pde_loss": float(history["pde_loss"][-1]),
        "final_bc_loss": float(history["bc_loss"][-1]),
        "final_ic_loss": float(history["ic_loss"][-1]),
    }
