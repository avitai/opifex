...
# Request enough samples that, after the loader drops the last partial
# batch (drop_remainder=True), at least ``n_total`` remain.
n_request = ((n_total + BATCH_SIZE - 1) // BATCH_SIZE) * BATCH_SIZE + BATCH_SIZE
...
base_param_count = sum(p.size for p in jax.tree_util.tree_leaves(nnx.state(base_fno, nnx.Param)))
...
pred_mean, half_width = predict_bands_in_batches(uqno, x_test_n, EVAL_BATCH_SIZE)
