...
def test_cascade_trainer_lifecycle():
    ...
    trainer.get_current_model().param.value = jnp.array([10.0, 10.0])  # pyright: ignore[reportAttributeAccessIssue]

    advanced = trainer.advance_level()
    assert advanced
    assert trainer.current_level == 1
    assert trainer.get_current_model().size == 4  # pyright: ignore[reportAttributeAccessIssue]

    # Check prolongation
    fine_vals = trainer.get_current_model().param.value  # pyright: ignore[reportAttributeAccessIssue]
