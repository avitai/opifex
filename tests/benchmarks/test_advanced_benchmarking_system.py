...
def _make_result(
    ...
    """Create a BenchmarkResult with the calibrax API from simple float metrics."""
...
class TestBenchmarkRegistry:
    ...
    def test_domain_config_creation(self):
        ...
        config = DomainConfig(name="fluid_dynamics")
...
class TestValidationFramework:
    ...
    def test_tolerance_violation_checking(self):
        ...
        # _check_tolerance_violations is a module-level function in the new API
        from opifex.benchmarking.validation_framework import (
            _check_tolerance_violations,
        )

        violations = _check_tolerance_violations(
            high_error_metrics, "fluid_dynamics_test", validator._infer_domain
        )
    ...
    def test_chemical_accuracy_assessment(self):
        ...
        assert assessment.achieved == 1e-4

        # Poor accuracy result
        poor_result = _make_result(
            name="test_dft",
            dataset="quantum_molecule",
            metrics={"mse": 1e-2},
            execution_time=1.0,
...
class TestAnalysisEngine:
    ...
    def test_performance_insights(self):
        ...
        excellent_result = _make_result(
            name="efficient_fno",
            dataset="test_problem",
...
class TestResultsManager:
    ...
    def test_result_persistence(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            assert loaded.tags.get("dataset") == "test_dataset"
            assert loaded.metrics["mse"].value == 1e-3
    ...
    def test_query_functionality(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            # Query by name
            fast_results = manager.query_results(name="fast_model")
            assert len(fast_results) == 1
            assert fast_results[0]["name"] == "fast_model"
...
class TestBenchmarkRunner:
    ...
    def test_single_benchmark_execution(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            # Mock operator -- must be nnx.Module with nnx.Param
            class MockFNO(nnx.Module):
                def __init__(self, rngs: nnx.Rngs, **_kwargs) -> None:
            ...
            assert result.name == "MockFNO"
            assert result.tags.get("dataset") == "exec_test_benchmark"
