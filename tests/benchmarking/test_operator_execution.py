...
class TestTrainingLoop:
    ...
    def test_benchmark_result_from_training(self):
        ...
        # After training, final loss should be <= initial (training improved or at least didn't diverge)
        assert (
            result.metrics["final_train_loss"]
            <= result.metrics["initial_train_loss"] * 1.5
        )
...
class TestBenchmarkRunnerRealExecution:
    ...
    def test_run_single_benchmark_returns_real_metrics(self):
        ...
        assert result.dataset_name == "test_darcy"
        assert result.metrics["mse"] > 0  # Real MSE (not random)
        ...
        assert abs(result.metrics["mse"] - result2.metrics["mse"]) < 0.1
