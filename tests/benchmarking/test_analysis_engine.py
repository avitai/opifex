...
- Performance rankings and winner determination
- Metric normalization (lower-is-better vs higher-is-better)
...
class TestAnalysisEngineInitialization:
    ...
    def test_metric_weights_defined(self):
        """Test that metric weights are defined for various domains."""
        engine = AnalysisEngine()

        assert "fluid_dynamics" in engine.metric_weights
        assert "quantum_computing" in engine.metric_weights
        assert "materials_science" in engine.metric_weights
        assert "default" in engine.metric_weights
...
class TestMetricNormalization:
    """Test metric normalization functionality."""

    def setup_method(self):
        """Set up test fixtures."""
        self.engine = AnalysisEngine()

    def test_normalize_lower_is_better_metric(self):
        """Test normalization for metrics where lower is better (mse, mae)."""
        metric_values = {"FNO": 0.01, "DeepONet": 0.05}

        # FNO should get score of 1.0 (best), DeepONet should get lower score
        fno_score = self.engine._normalize_metric_score("mse", "FNO", metric_values)
        deeponet_score = self.engine._normalize_metric_score(
            "mse", "DeepONet", metric_values
        )

        assert fno_score == pytest.approx(1.0, rel=1e-5)
        assert deeponet_score == pytest.approx(0.0, rel=1e-5)

    def test_normalize_higher_is_better_metric(self):
        """Test normalization for metrics where higher is better (r2_score)."""
        metric_values = {"FNO": 0.95, "DeepONet": 0.85}

        # FNO should get score of 1.0 (best), DeepONet should get lower score
        fno_score = self.engine._normalize_metric_score(
            "r2_score", "FNO", metric_values
        )
        deeponet_score = self.engine._normalize_metric_score(
            "r2_score", "DeepONet", metric_values
        )
    ...
    def test_normalize_equal_values(self):
        ...
        metric_values = {"FNO": 0.01, "DeepONet": 0.01}

        # All should get score of 1.0 when equal
        fno_score = self.engine._normalize_metric_score("mse", "FNO", metric_values)
        deeponet_score = self.engine._normalize_metric_score(
            "mse", "DeepONet", metric_values
        )
        ...
        assert fno_score == pytest.approx(1.0, rel=1e-5)
        assert deeponet_score == pytest.approx(1.0, rel=1e-5)
...
class TestComplexityEstimation:
    ...
    def test_linear_complexity_estimation(self):
        ...
        complexity = self.engine._estimate_complexity("execution_time", 1.0)
        assert complexity == "O(n)"

    def test_quadratic_complexity_estimation(self):
        """Test O(n^2) complexity estimation."""
        complexity = self.engine._estimate_complexity("execution_time", 2.0)
        assert complexity == "O(n²)"

    def test_cubic_complexity_estimation(self):
        """Test O(n^3) complexity estimation."""
        complexity = self.engine._estimate_complexity("execution_time", 3.0)
        assert complexity == "O(n³)"

    def test_higher_complexity_estimation(self):
        """Test higher complexity estimation."""
        complexity = self.engine._estimate_complexity("execution_time", 4.5)
    ...
    def test_non_execution_time_metric(self):
        ...
        complexity = self.engine._estimate_complexity("mse", 1.5)
...
class TestPerformanceInsightsGeneration:
    ...
    def test_high_execution_time_bottleneck(self):
        ...
        result = BenchmarkResult(
            ...
            dataset_name="TestDataset",
            metrics={"mse": 0.01},
            execution_time=15.0,  # High execution time
            framework_version="flax_nnx",
        )
    ...
    def test_exceptional_accuracy_insight(self):
        ...
        result = BenchmarkResult(
            model_name="FNO",
            dataset_name="TestDataset",
            metrics={"mse": 1e-7},  # Exceptional accuracy
            execution_time=1.0,
            framework_version="flax_nnx",
        )
...
class TestDomainInference:
    ...
    def test_quantum_computing_domain(self):
        ...
        assert self.engine._infer_domain("QuantumChemistry") == "quantum_computing"
        assert self.engine._infer_domain("dft_energies") == "quantum_computing"
        assert self.engine._infer_domain("molecular_dynamics") == "quantum_computing"

    def test_fluid_dynamics_domain(self):
        """Test inference of fluid dynamics domain."""
        assert self.engine._infer_domain("NavierStokes2D") == "fluid_dynamics"
        assert self.engine._infer_domain("Burgers1D") == "fluid_dynamics"
        assert self.engine._infer_domain("DarcyFlow") == "fluid_dynamics"

    def test_materials_science_domain(self):
        """Test inference of materials science domain."""
        assert self.engine._infer_domain("CrystalEnergies") == "materials_science"
        assert self.engine._infer_domain("SolidMechanics") == "materials_science"
        assert self.engine._infer_domain("material_properties") == "materials_science"

    def test_climate_modeling_domain(self):
        """Test inference of climate modeling domain."""
        assert self.engine._infer_domain("WeatherPrediction") == "climate_modeling"
        assert self.engine._infer_domain("climate_data") == "climate_modeling"
        assert self.engine._infer_domain("atmospheric_pressure") == "climate_modeling"

    def test_general_domain_fallback(self):
        """Test fallback to general domain."""
        assert self.engine._infer_domain("SomeRandomDataset") == "general"
        assert self.engine._infer_domain("TestDataset") == "general"
...
class TestStatisticalSignificanceTesting:
    ...
    def test_single_run_significance_testing(self):
        ...
        result2 = BenchmarkResult(
            model_name="DeepONet",
            dataset_name="TestDataset",
            metrics={"mse": 0.05},  # 5x difference
            execution_time=2.0,
            framework_version="flax_nnx",
        )

        significance = self.engine._test_statistical_significance(
            {"FNO": result1, "DeepONet": result2}
        )
    ...
    def test_multi_run_significance_testing(self):
        ...
        fno_results = [
            BenchmarkResult(
                model_name="FNO",
                dataset_name="TestDataset",
                metrics={"mse": 0.01 + np.random.normal(0, 0.001)},
                execution_time=1.0,
                framework_version="flax_nnx",
            )
            for _ in range(10)
        ...
        deeponet_results = [
            BenchmarkResult(
                model_name="DeepONet",
                dataset_name="TestDataset",
                metrics={"mse": 0.05 + np.random.normal(0, 0.001)},
                execution_time=2.0,
                framework_version="flax_nnx",
            )
            for _ in range(10)
...
class TestConfidenceCalculation:
    ...
    def test_confidence_base_level(self):
        ...
        result = BenchmarkResult(
            ...
            dataset_name="TestDataset",
            metrics={},  # No metrics
            execution_time=0,  # Invalid execution time
            framework_version="flax_nnx",
        )

        confidence = self.engine._calculate_confidence(result)
    ...
    def test_confidence_increases_with_good_metrics(self):
        ...
        result = BenchmarkResult(
            model_name="FNO",
            dataset_name="TestDataset",
            metrics={"mse": 1e-5},  # Good MSE
            execution_time=1.0,  # Reasonable execution time
            framework_version="flax_nnx",
        )
