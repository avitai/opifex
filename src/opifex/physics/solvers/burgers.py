...
def solve_burgers_1d(
    ...
    # CFL condition check
    max_u = jnp.max(jnp.abs(initial_condition))
    cfl = max_u * dt / dx

    # Adjust time step if necessary for stability
    dt = jnp.where(cfl > 0.5, 0.5 * dx / (max_u + 1e-8), dt)

    def step_forward(u):
        """Single time step using finite difference."""
        ...
        return u - dt * u * u_x + dt * viscosity * u_xx
...
def solve_burgers_2d(
    ...
    # CFL condition
    max_u = jnp.max(jnp.abs(initial_condition))
    dt = jnp.minimum(dt, 0.25 * dx**2 / viscosity)  # Diffusion stability
    dt = jnp.minimum(dt, 0.5 * dx / (max_u + 1e-8))  # Convection stability

    def step_forward(u):
        """Single time step for 2D Burgers equation."""
        # Periodic boundary conditions
        ...
        return u - dt * (u * u_x + u * u_y) + dt * viscosity * (u_xx + u_yy)
