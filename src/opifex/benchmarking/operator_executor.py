...
from opifex.benchmarking.evaluation_engine import BenchmarkResult, EvaluationMetrics
...
class OperatorExecutor:
    ...
    - Existing EvaluationMetrics (DRY)
    """
    ...
    def execute_training_benchmark(
        ...
        return BenchmarkResult(
            model_name=operator_class.__name__,
    ...
    def _train_loop(
        ...
        def prepare_input(x: jnp.ndarray) -> jnp.ndarray:
        ...
        def prepare_target(y: jnp.ndarray, pred: jnp.ndarray) -> jnp.ndarray:
        ...
        def train_step(model, x, y):
            ...
            def loss_fn(m):
                ...
                pred = m(x_input)
                y_target = prepare_target(y, pred)
    ...
    def _evaluate(
        ...
        DRY: Uses existing EvaluationMetrics from evaluation_engine.py
        ...
        # DRY: Reuse existing metrics implementation
        return {
            "mse": self._metrics_calculator.calculate_mse(preds, targets),
            "mae": self._metrics_calculator.calculate_mae(preds, targets),
            "relative_error": self._metrics_calculator.calculate_relative_error(
                preds, targets
            ),
        }
