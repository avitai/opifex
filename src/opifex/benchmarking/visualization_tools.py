...
class PDEBenchVisualizer:
    ...
    def create_comparison_chart(
        ...
        return {
            ...
            or metric.lower() in ["mse", "mae", "rmse", "relative_error", "mape"]
            else "descending",
            "chart_config": {
                "x_axis": "model_name",
                "y_axis": metric,
    ...
    def create_multi_metric_comparison(
        ...
        model_data = {}
        for result in results:
            model_key = f"{result.model_name}_{result.dataset_name}"
            ...
            for metric in metrics:
                if metric in result.metrics:
                    model_data[model_key]["metrics"][metric] = result.metrics[metric]
    ...
    def create_baseline_comparison(
        ...
        for result in results:
            ...
            for baseline_model, baseline_data in baseline_metrics.items():
                if baseline_model.lower() in result.model_name.lower():
            ...
            comparison_data.append(
                {
                    "model_name": result.model_name,
                    "dataset_name": result.dataset_name,
                    "test_value": result.metrics[metric],
                    "baseline_value": baseline_value,
                    "improvement": (baseline_value - result.metrics[metric])
                    / baseline_value
                    if baseline_value
                    else None,
                }
    ...
    def create_error_distribution(
        ...
        for result in results:
            if error_metric in result.metrics:
                error_value = result.metrics[error_metric]
                error_values.append(error_value)

                if result.model_name not in model_errors:
                    model_errors[result.model_name] = []
                model_errors[result.model_name].append(error_value)
    ...
    def create_model_ranking(
        ...
        for result in results:
            ...
            for metric in ranking_metrics:
                if metric in result.metrics and metric in weights:
                    value = result.metrics[metric]
            ...
            if total_weight > 0:
                model_scores[model_key] = {
                    ...
                    "individual_metrics": {
                        m: result.metrics.get(m) for m in ranking_metrics
                    },
                }
