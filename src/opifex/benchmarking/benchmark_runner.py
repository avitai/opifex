...
class BenchmarkRunner:
    ...
    def _get_data_loaders(self, benchmark_config: BenchmarkConfig):
    ...
    def _get_operator_config(
        ...
        metadata = self.registry._operator_metadata.get(operator_name, {})
        ...
        DEFAULT_CONFIGS: dict[str, dict[str, Any]] = {
            ...
            "deeponet": {
                "branch_sizes": [
                    int(jnp.prod(jnp.array(benchmark_config.input_shape))),
                    128,
                    64,
                ],
        ...
        summary.update(
            {
                ...
                "avg_mse": float(
                    jnp.mean(
                        jnp.array([mse for mse in mse_values if mse != float("inf")])
                    )
                ),
