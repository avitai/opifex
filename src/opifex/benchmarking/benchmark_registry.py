...
class DomainConfig:
    ...
    def __post_init__(self):
        ...
        if not self.tolerance_ranges:
            # Default tolerance ranges for different domains
            domain_defaults = {
                "fluid_dynamics": {
                    "mse": (1e-6, 1e-2),
                ...
                "quantum_computing": {
                    "mse": (1e-8, 1e-4),  # Higher accuracy required for quantum
                    "relative_error": (1e-6, 1e-3),
                    "chemical_accuracy": (1e-3, 5e-3),  # kcal/mol
                },
                "materials_science": {
                    "mse": (1e-7, 1e-3),
                    "formation_energy": (1e-2, 5e-2),  # eV/atom
                    "force_accuracy": (1e-2, 1e-1),  # eV/Å
                },
                "climate_modeling": {
                    "mse": (1e-5, 1e-2),
            ...
            self.tolerance_ranges = domain_defaults.get(
                self.name,
                {
                    "mse": (1e-6, 1e-2),
                    "relative_error": (1e-4, 1e-1),
        ...
        if not self.default_problem_sizes:
            self.default_problem_sizes = [64, 128, 256, 512]
...
class BenchmarkRegistry:
    ...
    def _setup_default_domains(self) -> None:
        ...
        domains = [
            DomainConfig("fluid_dynamics"),
            DomainConfig("quantum_computing"),
            DomainConfig("materials_science"),
            DomainConfig("climate_modeling"),
            DomainConfig("molecular_dynamics"),
            DomainConfig("plasma_physics"),
        ]
    ...
    def _update_compatibility(self, benchmark: BenchmarkConfig) -> None:
        ...
        for operator_name, operator_class in self._operators.items():
            # Simple heuristic: if operator can handle the input shape
            try:
                # This is a basic check - more sophisticated compatibility
                # checking can be implemented based on operator capabilities
                if callable(operator_class):
                    self._benchmark_operator_compatibility[benchmark.name].add(
                        operator_name
                    )
            except Exception:
                # Skip operators that can't be introspected
                pass
    ...
    def auto_discover_operators(self) -> None:
        ...
        try:
            ...
            for operator_class in operator_classes:
                if operator_class.__name__ not in self._operators:
                    self.register_operator(operator_class)

        except ImportError as e:
            logger.warning(f"Could not auto-discover operators: {e}")
    ...
    def generate_compatibility_report(self) -> dict[str, Any]:
        ...
        for operator_name in self._operators:
            coverage_count = sum(
                1
                for benchmark_name in self._benchmarks
