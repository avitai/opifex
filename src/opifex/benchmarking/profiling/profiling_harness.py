...
class OptimizationReport:
    ...
    def __init__(self) -> None:
        """Initialize empty optimization report."""
        self.sections: dict[str, Any] = {}
        self.executive_summary: dict[str, Any] = {}
        self.priority_recommendations: list[dict[str, str]] = []

    def add_section(self, title: str, content: Any) -> None:
        """Add a section to the report."""
        self.sections[title] = content

    def set_executive_summary(self, summary: dict[str, Any]) -> None:
...
class OpifexProfilingHarness:
    ...
    def profile_neural_operator(
        ...
        op_fn = cast("Callable[..., Any]", operator)

        # Roofline analysis (calibrax RooflineAnalyzer)
        if "roofline" in self.profilers:
            try:
                roofline_result = self.roofline_analyzer.analyze_operation(
                    op_fn,
                    list(inputs),
                )
                results["roofline_analysis"] = roofline_result.to_dict()
            except (RuntimeError, ValueError, TypeError) as e:
                results["roofline_analysis"] = {"error": str(e)}

        # Compilation analysis (calibrax CompilationProfiler)
        if "compilation" in self.profilers:
            try:
                instrumented = self.compilation_profiler.profile_jit_compilation(
                    op_fn,
                )
                instrumented(*inputs)
                comp_result = self.compilation_profiler.get_result()
                results["compilation_analysis"] = comp_result.to_dict()
            except (RuntimeError, ValueError, TypeError) as e:
