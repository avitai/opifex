...
# Enable 64-bit precision explicitly via config
...
def test_training_infrastructure():
    ...
    config = TrainingConfig(learning_rate=0.001, num_epochs=10, batch_size=32)
...
def test_activation_functions():
    ...
    available = list_activations()
    assert len(available) > 10, "Should have many activation functions"
    assert "relu" in available, "ReLU should be available"
    assert "snake" in available, "Snake activation should be available"

    # Test scientific activations
    x = jnp.array([0.0, 1.0, 2.0])
    snake_output = snake_activation(x)
    assert snake_output.dtype in (jnp.float32, jnp.float64), (
        "Activation should preserve floating point precision"
    )

    # Test normalized tanh
    norm_tanh_output = normalized_tanh(x)
    assert norm_tanh_output.dtype in (jnp.float32, jnp.float64), (
        "Normalized tanh should be floating point"
    )
...
def test_environment_configuration():
    ...
    # Float64 may be truncated to float32 on GPU without x64 mode
    x64 = jnp.float64(1.0)
    assert x64.dtype in (jnp.float32, jnp.float64), "Float64 or float32 precision"
