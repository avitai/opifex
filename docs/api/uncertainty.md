...
| `opifex.uncertainty.metrics` | Deprecated, removed in 0.2.3: keyword wrappers over `calibrax.metrics.functional.uncertainty` (`predictive_entropy`, `ensemble_mutual_information`, `interval_score`, `winkler_score`, `anees`, `non_credibility_index`, `chi2_confidence_interval`). Each call emits a `DeprecationWarning`; call calibrax directly. |
| `opifex.uncertainty.forecasting_metrics` | Deprecated, removed in 0.2.3: keyword wrappers over `calibrax.metrics.functional.forecasting` (`fair_crps`, `energy_score`, `rank_histogram`, `spread_skill_ratio`, `pit_histogram`, `ranked_probability_score`, `event_reliability`, `ensemble_ranked_probability_score`, `ranked_probability_skill_score`) and `calibrax.metrics.functional.regression.crps`. Each call emits a `DeprecationWarning`; call calibrax directly. |
