...
4. **Gradient checkpointing**: Add `@jax.checkpoint` to spectral layers for 3-5x memory savings at higher resolutions
...
# Option 2: Enable gradient checkpointing on the forward pass
@jax.checkpoint
def forward(model, x):
    return model(x)
